...
func main() {
	...
	for _, idlFilepath := range filenames {
		...
		for _, file := range files {
			...
			file.File.HeaderComment("Code generated by https://github.com/daog1/solana-anchor-go. DO NOT EDIT.")
			{
				// Save assets:
...
func DecodeEventsInLogs(logMessages []string) (evts []*Event, err error) {
	var base64Binaries [][]byte
	logMessageEventBinaries, err := decodeEventsFromLogMessage(logMessages)
	if err != nil {
		return
	}
...
`))
	...
	// add constants file"
	{
		...
		for _, c := range idl.Constants {
			...
			case "bytes":
				v, err := BytesStringToString(c.Value)
				if err != nil {
					panic(fmt.Sprintf("failed to parse constant: %s", spew.Sdump(c)))
				}
